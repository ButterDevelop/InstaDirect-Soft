...
{
    public class InstagramAvatarChanger
    {
        public Image Avatar;
        ...
        public void Log(string x)
        {
            try
            {
                try
                {
                    File.AppendAllText(Path.Combine(path, "InstagramAvatarChanger.log"), "", Encoding.UTF8);
                }
                catch { }
                string res = "[" + DateTime.Now.ToString() + ", поток " + ThreadNumber + "] " + x + Environment.NewLine;
                log += res;
                File.AppendAllText(Path.Combine(path, "InstagramAvatarChanger.log"), res, Encoding.UTF8);
            }
            catch (Exception e) { /*MessageBox.Show(e.ToString());*/ }
        }

        public InstagramAvatarChanger(string Account, int indx, int tt, Image ava)
        {
            Avatar = ava;
            ...
            try
            {
                ...
                } else
                {
                    ...
                    if (ChangeAvatar(logincookie)) { DoResult(Account); return; }
                    logincookie = DoLogin(cookie);
                }

                if (ChangeAvatar(logincookie)) { Log(Translate.Tr("Сменили аватарку - ") + Account); DoResult(Account); return; } else Log(Account + Translate.Tr("- что-то пошло не так!"));
            }
        ...
        public bool ChangeAvatar(string cookie)
        {
            try
            {
                ...
                using (var request = new xNet.HttpRequest())
                {
                    ...
                    request.UserAgent = UserAgent;
                    request.KeepAlive = true;
                    //request.Referer = $"https://www.instagram.com/{Login}/";
                    //request.Proxy = HttpProxyClient.Parse("127.0.0.1:8888");
                    if (proxytype == "HTTP") request.Proxy = HttpProxyClient.Parse(Proxy[index % Proxy.Count]); else request.Proxy = Socks5ProxyClient.Parse(Proxy[index % Proxy.Count]);

                    request.AddHeader("Origin", "https://www.instagram.com");
                    request.AddHeader("X-Instagram-AJAX", "c86f62fe51e1");

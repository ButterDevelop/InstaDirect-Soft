...
namespace InstaDirectMessage_ButDev
{
    public partial class FormHashtagsParser : Form
    {
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            if (HashtagsPath == "") { MessageBox.Show(Translate.Tr("Файл с хэштегами не указан!"), Translate.Tr("Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            ...
            stop = false;
            InstagramHashtagsParser.stop = false;
            ...
            InstagramHashtagsParser.path = path;

            ResultPathStatic = ResultPath;
            InstagramHashtagsParser.GoodPath = ResultPathStatic;
            if (HowManyID == 0) InstagramHashtagsParser.HowManyID = int.MaxValue; else InstagramHashtagsParser.HowManyID = HowManyID;
            InstagramHashtagsParser.proxytype = comboBoxTypeProxy.Text;
            InstagramHashtagsParser.resulttype = comboBoxResultFormat.SelectedIndex;
            ...
            InstagramHashtagsParser.Proxy = Proxy;

            //Load Hashtags
            using (StreamReader sr = new StreamReader(HashtagsPath, true))
            {
                string line;
                while ((line = sr.ReadLine()) != null)
                {
            ...
            InstagramHashtagsParser.UserAgents = UserAgents;

            labelAll.Text = Hashtags.Count.ToString();
            ...
            if (ThreadsCount > Hashtags.Count) ThreadsCount = Hashtags.Count;
            int L = 0; int R = Hashtags.Count / ThreadsCount;
            for (int potok = 0; potok < ThreadsCount; potok++)
            {
                new Thread(() => Work(L, R, potok, progress, log, setGood)).Start();
                Thread.Sleep(300);
                L = R;
                R += Hashtags.Count / ThreadsCount;
            }
            if (Hashtags.Count % ThreadsCount != 0) { finalresult = ThreadsCount + 1; new Thread(() => Work(R, Hashtags.Count, ThreadsCount, progress, log, setGood)).Start(); } else finalresult = ThreadsCount;
        ...
        private void WaitForEnd(IProgress<int> vis)
        {
            ...
            MessageBox.Show(Translate.Tr("Завершено!"), Translate.Tr("Парсер хэштегов"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
            vis.Report(999);
            stop = true;
        }
        ...
        {
            if (!File.Exists(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramHashtagsParser.log"))) MessageBox.Show(Translate.Tr("Файла с логом пока нет!"), Translate.Tr("Парсер хэштегов: Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Process.Start(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramHashtagsParser.log"));
        }
        ...
        {
            stop = true;
            InstagramHashtagsParser.stop = true;
            buttonStop.Visible = false;
            Thread.Sleep(ThreadsCount * 500);
            buttonStart.Visible = true;
            MessageBox.Show(Translate.Tr("Вы нажали кнопку \"Стоп\"."), Translate.Tr("Парсер хэштегов"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
        }

        private void buttonOpenHashtags_Click(object sender, EventArgs e)
        {
            ...
            textBoxHashtags.Text = openFileDialog.FileName;
            HashtagsPath = openFileDialog.FileName;
        }

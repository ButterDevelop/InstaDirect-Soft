...
{
    public partial class FormFollowersParser : Form
    {
        public static string ProxyPath = "", IDPath = "", AccountsPath = "", ResultPath = "", ResultPathStatic = "", PROXYTYPE = "", time = "", ApiProxyUrl = "";
        ...
        {
            Final = 0;
            Proxy.Clear(); IDrival.Clear(); Accounts.Clear();
            ...
            if (IDPath == "") { MessageBox.Show(Translate.Tr("Файл с ID конкурентов не указан!"), Translate.Tr("Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            ...
            stop = false;
            InstagramFollowersParser.stop = false;
            ...
            InstagramFollowersParser.path = path;

            ResultPathStatic = ResultPath;
            InstagramFollowersParser.GoodPath = ResultPathStatic;
            if (HowManyID == 0) InstagramFollowersParser.HowManyID = int.MaxValue; else InstagramFollowersParser.HowManyID = HowManyID;
            InstagramFollowersParser.proxytype = comboBoxTypeProxy.Text;
            InstagramFollowersParser.resulttype = comboBoxResultFormat.SelectedIndex;
            ...
            InstagramFollowersParser.Proxy = Proxy;

            //Load ID
            using (StreamReader sr = new StreamReader(IDPath, Encoding.UTF8))
            {
                string line;
                while ((line = sr.ReadLine()) != null)
                {
            ...
            InstagramFollowersParser.Accounts = Accounts;

            //Load User Agents
            var UserAgents = Properties.Resources.UserAgents.Split(new char[1] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
            InstagramFollowersParser.UserAgents = UserAgents;

            labelAll.Text = IDrival.Count.ToString();
            ...
            if (ThreadsCount > IDrival.Count) ThreadsCount = IDrival.Count;
            int L = 0; int R = IDrival.Count / ThreadsCount;
            for (int potok = 0; potok < ThreadsCount; potok++)
            {
                new Thread(() => Work(L, R, potok, progress, log, setGood)).Start();
                Thread.Sleep(300);
                L = R;
                R += IDrival.Count / ThreadsCount;
            }
            if (IDrival.Count % ThreadsCount != 0) { finalresult = ThreadsCount + 1; new Thread(() => Work(R, IDrival.Count, ThreadsCount, progress, log, setGood)).Start(); } else finalresult = ThreadsCount;
        ...
        private void WaitForEnd(IProgress<int> vis)
        {
            ...
            MessageBox.Show(Translate.Tr("Завершено!"), Translate.Tr("Парсер подписчиков"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
            vis.Report(999);
            stop = true;
        }
        ...
        {
            if (!File.Exists(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramFollowersParser.log"))) MessageBox.Show(Translate.Tr("Файла с логом пока нет!"), Translate.Tr("Парсер подписчиков: Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Process.Start(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramFollowersParser.log"));
        }
        ...
        private void MainThread(int L, int R, int tt, int indx, int indexAcc, IProgress<int> progress, IProgress<string> log, IProgress<int> setGood)
        {
            for (int i = L; i < R; i++)
            {
                ...
                {
                    if (stop) return;
                    if (i >= IDrival.Count) break;

                    InstagramFollowersParser parser = new InstagramFollowersParser(IDrival[i], indx, indexAcc, tt, setGood);
        ...
        {
            stop = true;
            InstagramFollowersParser.stop = true;
            buttonStop.Visible = false;
        ...
        private void buttonOpenRivals_Click(object sender, EventArgs e)
        {
            OpenFileDialog openFileDialog = new OpenFileDialog();
            openFileDialog.Filter = "Text files(*.txt)|*.txt|All files(*.*)|*.*";
            if (openFileDialog.ShowDialog() == DialogResult.Cancel)
            ...
            textBoxRivals.Text = openFileDialog.FileName;

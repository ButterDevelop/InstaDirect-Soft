...
{
    public class InstagramMassFollowing
    {
        public Image Avatar;
        public int index = 0, ThreadNumber = 0, good = 0, GoodCount = 0;
        ...
        public void Log(string x)
        {
            try
            {
                try
                {
                    File.AppendAllText(Path.Combine(path, "InstagramMassFollowing.log"), "", Encoding.UTF8);
                }
                catch { }
                string res = "[" + DateTime.Now.ToString() + ", поток " + ThreadNumber + "] " + x + Environment.NewLine;
                log += res;
                File.AppendAllText(Path.Combine(path, "InstagramMassFollowing.log"), res, Encoding.UTF8);
            }
            catch (Exception e) { /*MessageBox.Show(e.ToString());*/ }
        }

        public InstagramMassFollowing(string Account, int indx, int tt)
        {
            ...
            try
            {
                ...
                } else
                {
                    ...
                    if (Follow(logincookie)) { DoResult(Account); return; }
                    logincookie = DoLogin(cookie);
                }
        ...
        public bool Follow(string cookie)
        {
            ...
            try
            {
                ...
                if (html.Contains("requested")) Log(Translate.Tr("Отправлен запрос на подписку с аккаунта ") + Login + Translate.Tr(" на аккаунт ") + ID[i]);
                else Log(Translate.Tr("Успешно подписались с аккаунта ") + Login + Translate.Tr(" на аккаунт ") + ID[i]);

                GoodCount++;
                if (i != ID.Count - 1) { Log(Translate.Tr("Ждём секунд: ") + Delay); Thread.Sleep(Delay * 1000); }
            }
            catch (Exception ex)
            {
                Log(Translate.Tr("(Ошибка) Текст ошибки: ") + ex.ToString());

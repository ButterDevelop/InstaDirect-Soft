...
{
    public partial class FormWebRegistrator : Form
    {
        public static string ProxyPath = "", NamePath = "", SurnamePath = "", UAsPath = "", ResultFilePath = "", ResultUAFilePath = "", PROXYTYPE = "", time = "", ApiProxyUrl = "", MailsPath = "", 
                             IMAPPath = "", ResultCheckpointPath = "", StaticMailsPath = "";
        ...
        public FormWebRegistrator()
        {
            ...
            var data = Properties.Settings.Default.FormWebRegistrator;
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            labelBad.Text = labelCheckpoint.Text = labelGood.Text = "0";
            ...
            if (MailsPath == "") { MessageBox.Show(Translate.Tr("Файл с почтами не указан!"), Translate.Tr("Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            if (IMAPPath == "") { MessageBox.Show(Translate.Tr("Файл с IMAP не указан!"), Translate.Tr("Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            ...
            Properties.Settings.Default.FormWebRegistrator = data;
            Properties.Settings.Default.Save();

            InstagramWebRegistrator.CheckpointPath = ResultCheckpointPath;
            InstagramWebRegistrator.ResultPath = ResultFilePath;
            InstagramWebRegistrator.ResultUAPath = ResultUAFilePath;
            ...
            InstagramWebRegistrator.path = path;

            InstagramWebRegistrator.proxytype = comboBoxTypeProxy.Text;

            if (!checkBoxRegionAutoDetection.Checked) InstagramWebRegistrator.AcceptLang = Locales[comboBoxRegion.SelectedIndex]; else InstagramWebRegistrator.AcceptLang = "autodetect";
            ...
            InstagramWebRegistrator.Proxy = Proxy;
            ...
            InstagramWebRegistrator.Names = Names;
            ...
            InstagramWebRegistrator.Surnames = Surnames;
            ...
            InstagramWebRegistrator.IMAPKey = IMAPKey;
            InstagramWebRegistrator.IMAPValue = IMAPValue;

            //Load User Agents
            //var UserAgents = Properties.Resources.UserAgents.Split(new char[1] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
            var UserAgents = File.ReadAllLines(UAsPath);
            InstagramWebRegistrator.UserAgents = UserAgents;
        ...
        private void WaitForEnd(IProgress<int> vis)
        {
            ...
            MessageBox.Show(Translate.Tr("Завершено!"), Translate.Tr("Веб-регистратор"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
            vis.Report(999);
            stop = true;
        }
        ...
        {
            if (!File.Exists(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstaWebRegistrator.log"))) MessageBox.Show(Translate.Tr("Файла с логом пока нет!"), Translate.Tr("Веб-регистратор: Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Process.Start(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstaWebRegistrator.log"));
        }
        ...
        private void MainThread(int L, int R, int tt, int indx, IProgress<int> progress, IProgress<string> log)
        {
            for (int i = L; i < R; i++)
            {
                ...
                try
                {
                    ...
                    InstagramWebRegistrator reg = new InstagramWebRegistrator(dict[0], dict[1], name, surname, indx + tries[tt], tt);
                    log.Report(reg.log);
                    if (reg.good == 1) { BlackList.Add(Mails[i]); progress.Report(1); tries[tt] = 0; }
                    else
        ...
        private void buttonStop_Click(object sender, EventArgs e)
        {
            ...
            buttonStart.Visible = true;
            MessageBox.Show(Translate.Tr("Вы нажали кнопку \"Стоп\"."), Translate.Tr("Веб-регистратор"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
        }

...
namespace InstaDirectMessage_ButDev
{
    public partial class FormSMSApiRegistrator : Form
    {
        ...
        public static bool stop = true, isApiProxy = false, isFakeMails = true;

        public FormSMSApiRegistrator()
        {
            InitializeComponent();
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            InstagramSMSApiRegistrator.ResultUAPath = ResultUAFilePath;
            InstagramSMSApiRegistrator.CurrentMobileAppVersion = comboBoxCurrentInstagramVersion.Text;
            InstagramSMSApiRegistrator.APIKey = textBoxAPIKey.Text;
            InstagramSMSApiRegistrator.CountryCode = comboBoxPhoneRegion.Text.Split(new char[1] { ' ' })[0];

            if (comboBoxService.Text == "https://sms-activate.ru/") InstagramSMSApiRegistrator.Host = "https://sms-activate.ru/";
            else
                if (comboBoxService.Text == "sms.kopeechka.store") InstagramSMSApiRegistrator.Host = "http://213.166.69.162/";
            else
                if (comboBoxService.Text == "http://smsvk.net/") InstagramSMSApiRegistrator.Host = "http://smsvk.net/";
            else
                if (comboBoxService.Text == "sms-online.pro") InstagramSMSApiRegistrator.Host = "https://sms-online.pro/"; else InstagramSMSApiRegistrator.Host = "http://194.87.238.180/";

            if (!checkBoxRegionAutoDetection.Checked) InstagramSMSApiRegistrator.AcceptLang = Locales[comboBoxRegion.SelectedIndex]; else InstagramSMSApiRegistrator.AcceptLang = "autodetect";
            ...
            InstagramSMSApiRegistrator.path = path;

            InstagramSMSApiRegistrator.proxytype = comboBoxTypeProxy.Text;
            ...
            InstagramSMSApiRegistrator.Proxy = Proxy;

            //Load Mobile UserAgents
            //var UserAgentsMob = Properties.Resources.UserAgentMob.Split(new char[2] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries); ;
            //InstagramSMSApiRegistrator.UserAgentsMob = UserAgentsMob.ToList();
            ...
            InstagramSMSApiRegistrator.UserAgentsMob = UserAgentsMob;
            ...
            InstagramSMSApiRegistrator.Names = Names;
            ...
            InstagramSMSApiRegistrator.Surnames = Surnames;
        ...
        private void WaitForEnd(IProgress<int> vis)
        {
            ...
            MessageBox.Show(Translate.Tr("Завершено!"), Translate.Tr("SMS API регистратор"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
            vis.Report(999);
            stop = true;
        }
        ...
        {
            if (!File.Exists(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstaSMSApiRegistrator.log"))) MessageBox.Show(Translate.Tr("Файла с логом пока нет!"), Translate.Tr("SMS API регистратор: Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Process.Start(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstaSMSApiRegistrator.log"));
        }
            ...
            for (int i = 0; i < count; i++)
            {
                try
                {
                    while (!stop)
                    {
                        ...
                        InstagramSMSApiRegistrator reg = new InstagramSMSApiRegistrator(name, surname, phone, id, indx + tries[tt], tt);
                        log.Report(reg.log);
                        if (InstagramSMSApiRegistrator.Host == "http://213.166.69.162/") { reg.PhoneNumber = ""; phone = ""; id = ""; }
        ...
        private void buttonStop_Click(object sender, EventArgs e)
        {
            ...
            buttonStart.Visible = true;
            MessageBox.Show(Translate.Tr("Вы нажали кнопку \"Стоп\"."), Translate.Tr("SMS API регистратор"), MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
        }

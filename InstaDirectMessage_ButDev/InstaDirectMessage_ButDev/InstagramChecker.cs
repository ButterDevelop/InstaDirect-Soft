...
namespace InstaDirectMessage_ButDev
{
    public class InstagramChecker
    {
        ...
        public string UserAgent = "", csrfGlobal = "", log = "", Nick = "", FullNickname = "";
        public static List<string> Proxy = new List<string>();
        public static string[] UserAgents;
        public static string path = "", proxytype = "", GoodPath = "", BadPath = "";
        ...
        {
            File.AppendAllText(GoodPath, s + Environment.NewLine, Encoding.UTF8);
            good = 1;
        }
        ...
        public void Log(string x)
        {
            try
            {
                try
                {
                    File.AppendAllText(Path.Combine(path, "InstagramChecker.log"), "", Encoding.UTF8);
                }
                catch { }
                string res = "[" + DateTime.Now.ToString() + Translate.Tr(", поток ") + ThreadNumber + "] " + x + Environment.NewLine;
                log += res;
                File.AppendAllText(Path.Combine(path, "InstagramChecker.log"), res, Encoding.UTF8);
            }
            catch (Exception e) { /*MessageBox.Show(e.ToString());*/ }
        }

        public InstagramChecker(string nickname, int indx, int tt)
        {
            ...
            if (Nick.Contains(":")) Nick = Nick.Remove(Nick.IndexOf(":"));
        ...
        public int Check(string cookie)
        {
            try
            {
                string html = "";
                HttpStatusCode code = HttpStatusCode.None;
                using (var request = new xNet.HttpRequest())
                {
                    ...
                    try
                    {
                        var response = request.Get("https://www.instagram.com/web/search/topsearch/?query=" + Nick);
                    ...
                    {
                        DoBad();
                        Log(Translate.Tr("Аккаунт не найден - ") + Nick);
                    }

                    if (code == HttpStatusCode.OK) return 1; else return 2;
                }
            }
            catch (Exception ex)
            {

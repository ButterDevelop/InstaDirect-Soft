...
{
    public partial class FormPhotoWebPosting : Form
    {
        public static string ProxyPath = "", AccountsPath = "", ResultPath = "", ImagesPath = "", PROXYTYPE = "", time = "", ApiProxyUrl = "";
        public static int ThreadsCount = 0, Final = 0, finalresult = 0, HowManyID = 0, ApiProxyTimeout = 600, PostsNumber = 0, Delay = 0;
        ...
        public FormPhotoWebPosting()
        {
            ...
            var data = Properties.Settings.Default.FormPhotoWebPosting;
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(textBoxPostsNumber.Text, out PostsNumber) || PostsNumber <= 0) { MessageBox.Show(Translate.Tr("Количество постов указано некорректно!"), Translate.Tr("Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            ...
            Properties.Settings.Default.FormPhotoWebPosting = data;
            Properties.Settings.Default.Save();

            buttonStart.Visible = false;
            buttonStop.Visible = true;
            stop = false;
            InstagramPhotoWebPosting.stop = false;
            ShouldUniquePhoto = checkBoxShouldPhotoUnique.Checked;
            InstagramPhotoWebPosting.ShouldUniquePhoto = ShouldUniquePhoto;
            ...
            InstagramPhotoWebPosting.path = path;
            InstagramPhotoWebPosting.ResultPath = ResultPath;
            InstagramPhotoWebPosting.proxytype = comboBoxTypeProxy.Text;
            InstagramPhotoWebPosting.Delay = Delay;
            InstagramPhotoWebPosting.PostsNumber = PostsNumber;
            ...
            InstagramPhotoWebPosting.Images = Images;
            ...
            InstagramPhotoWebPosting.Proxy = Proxy;
            ...
            InstagramPhotoWebPosting.UserAgents = UserAgents;
            ...
            Progress<int> progress = new Progress<int>(p =>
            {
                ...
                labelBad.Text = (int.Parse(labelBad.Text) + PostsNumber - p).ToString();
        ...
        {
            if (!File.Exists(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramPhotoWebPosting.log"))) MessageBox.Show(Translate.Tr("Файла с логом пока нет!"), Translate.Tr("Постинг фото (web): Ошибка!"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Process.Start(Path.Combine(Path.Combine(Path.Combine(Environment.CurrentDirectory, time)), "InstagramPhotoWebPosting.log"));
        }
        ...
        private void MainThread(int L, int R, int tt, int indx, int indexAcc, IProgress<int> progress, IProgress<string> log)
        {
            for (int i = L; i < R; i++)
            {
                try
                {
                    ...
                    InstagramPhotoWebPosting worker = new InstagramPhotoWebPosting(Accounts[i], indx, tt);
        ...
        {
            stop = true;
            InstagramPhotoWebPosting.stop = true;
            buttonStop.Visible = false;
